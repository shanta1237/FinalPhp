...
if (isset($_POST['insert-bus'])) {
	...
	if ($admin == "" || $category == "" || $source == "" || $destination == "" || $title == "" || $intermediate == "" || $date == "" || $via_time == "" || $bus_detail == "" || $query_count == "" || $max_seats == "") {
		...
		VALUES({$category}, '{$title}', '{$admin}', '{$date}', '{$image}', '{$bus_detail}', '{$source}', '{$destination}', '{$intermediate}', '{$via_time}', '{$query_count}',$max_seats, $max_seats)";
...
<form action="#" method="post" enctype="multipart/form-data">
	...
	<div class="form-group">
		<label for="query count">Query count</label>
		<input type="text" class="form-control" name="count" placeholder="Either 0 or 1">
	</div>
